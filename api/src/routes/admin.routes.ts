...
// GET /api/admin/comments - Retrieve reported or blocked comments for moderation
app.get('/comments', authMiddleware, adminMiddleware, async (c) => {
  try {
    const { data: comments, error } = await supabaseAdmin
      ...
      `)
      .or('is_reported.eq.true,status.eq.rejected')
      .order('created_at', { ascending: false });
    ...
    return c.json({ error: 'Erro ao buscar comentários para moderação' }, 500);
  }
});

// PUT /api/admin/comments/:id/moderate - Approve or reject/block a comment
const moderateCommentSchema = z.object({
  status: z.enum(['approved', 'rejected']),
  reason: z.string().optional(),
});

app.put('/comments/:id/moderate', authMiddleware, adminMiddleware, zValidator('json', moderateCommentSchema), async (c) => {
  const commentId = c.req.param('id');
  const { status, reason } = c.req.valid('json');

  try {
    // 1. Fetch target comment
    const { data: targetComment, error: fetchError } = await supabaseAdmin
      .from('agency_comments')
      .select('*')
      .eq('id', commentId)
      .single();

    if (fetchError || !targetComment) {
    ...
    if (status === 'rejected') {
      updatePayload.content = 'Conteúdo removido pelo moderador por violar as diretrizes da comunidade.';
      updatePayload.moderation_reason = reason || 'Violação das regras da comunidade';
      updatePayload.is_reported = false; // resolved
    } else {
      // Re-approve: clear report status
      updatePayload.is_reported = false;
    }
    ...
    return c.json({ success: true, message: `Comentário ${status === 'rejected' ? 'rejeitado/bloqueado' : 'aprovado'} com sucesso` });
  } catch (err) {
    console.error('Admin moderate comment error:', err);
