...
export const updateProfileDefinition = {
  ...
  function: {
    ...
    parameters: {
      type: 'object',
      properties: {
        full_name: { type: 'string', description: 'Nome completo do usuário' },
        phone: { type: 'string', description: 'Telefone de contato' },
        linkedin_url: { type: 'string', description: 'URL completa do perfil do LinkedIn' },
        bio: { type: 'string', description: 'Biografia ou resumo profissional' },
        course: { type: 'string', description: 'Curso de graduação/estudos' },
        university: { type: 'string', description: 'Nome da faculdade, universidade ou instituição de ensino' },
        period: { 
          type: 'string', 
          enum: ['1-2', '3-5', '6+', 'formado'],
          description: 'Período acadêmico atual: "1-2" (1º - 2º período), "3-5" (3º - 5º período), "6+" (6º período ou mais) ou "formado".'
        },
      },
    },
  },
